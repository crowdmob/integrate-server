...
0. The Lulzio integrator launches an install campaign with CrowdMob.  *(You have to do this.)*
1. A user installs the CrowdMob store app.
2. On first run, the CrowdMob store app registers the user’s device with the CrowdMob server.
3. The user installs the Lulzio app through the CrowdMob store.
4. On first run, the Lulzio app registers the user’s device with the Lulzio server.  *(You have to do this.)*
5. The Lulzio server reports the install to the CrowdMob server.  *(You have to do this.)*
6. The CrowdMob server provides stats to the Lulzio integrator.
